...
if ($type == "top") {
	...
	while ( $top = $db->get_row ( $top_week ) ) {
		...
		if ($top ['song_id']) {
			$row = $db->super_query ( "SELECT vass_songs.artist_id, vass_songs.artists_id, vass_songs.id AS song_id, vass_songs.loved, vass_songs.title AS song_title,
			...
			$songs ['feat_artists'] = feat_artists ( $row ['artists_id'] );
			$songs ['buy_link'] = null;
...
if (! $member_id) {
	...
	if(isset($_SESSION['country_Id'])) {
		...
		$release = $db->query ( "SELECT vass_songs.id AS song_id, vass_songs.artist_id,
			...
			vass_artists.id AS artist_id, vass_artists.name AS song_artist, vass_albums.name AS song_album, vass_albums.id AS album_id
			FROM vass_songs LEFT JOIN vass_albums ON vass_songs.album_id = vass_albums.id LEFT JOIN
			vass_artists ON vass_songs.artist_id = vass_artists.id WHERE vass_songs.recent = 1 ORDER BY created_on DESC LIMIT 0,20" );
	...
	$buffer ['new_release'] = $result ['songs'];
	unset ( $buffer ['user'] ['password'] );

	// Top Week songs for home page
	if(isset($_SESSION['country_Id'])){
		$top_date = trim( $_REQUEST ['date'] );

		$D_SORT = 1;

		$top_week = $db->query ( "SELECT COUNT(*) AS count, song_id FROM vass_analz WHERE `time` > '" . date ( "Y-m-d", (time () - $D_SORT * 24 * 3600) ) . "' GROUP BY song_id ORDER by count DESC LIMIT 0,20" );
	...
	$end = $_REQUEST [results];

		$genre = $db->safesql($_REQUEST['genre']);
		if(isset($_SESSION['country_Id'])) {
			$genre_id = $db->super_query("SELECT id FROM vass_genres WHERE name LIKE '%". $genre . "%'");

			$trending_day = $db->query ( "SELECT COUNT(*) AS count, song_id FROM vass_analz WHERE `time` > '" . date ( "Y-m-d", (time ()  - 7*24*3600) ) . "' GROUP BY song_id ORDER by count DESC LIMIT 0,$end" );
...
// 		echo"<pre>"; print_r($artist_id); echo"</pre>"; die;
	...
	if (! $_REQUEST ['songid']) {
		...
		$row = $db->super_query ( "SELECT vass_songs.artist_id, vass_songs.artists_id, vass_songs.created_on, vass_songs.artist_id, vass_artists.tag AS tags, vass_songs.id AS song_id, vass_songs.title AS song_title,
...
//         echo  $_SESSION['user_id'];
        ...
        if (isset($_SESSION['country_Id'])) {
            ...
            $sql_query = $db->query ( "SELECT vass_songs.artist_id, vass_songs.artists_id, vass_songs.id AS song_id, vass_songs.loved, vass_songs.title AS song_title,
				vass_artists.id AS artist_id, vass_artists.name AS song_artist, vass_albums.name AS song_album, vass_albums.id AS album_id 
				FROM vass_songs LEFT JOIN vass_albums ON vass_songs.album_id = vass_albums.id LEFT JOIN 
	...
	if(!empty($letter)){
		...
		$artists = $db->query("SELECT id, name FROM vass_artists ORDER BY id DESC LIMIT $start,$end");
...
elseif ($type == "newrelease") {       
			...
			vass_artists.id AS artist_id, vass_artists.name AS song_artist, vass_albums.name AS song_album, vass_albums.id AS album_id 
			FROM vass_songs LEFT JOIN vass_albums ON vass_songs.album_id = vass_albums.id LEFT JOIN 
			vass_artists ON vass_songs.artist_id = vass_artists.id WHERE vass_songs.recent = 1 ORDER BY vass_songs.id DESC LIMIT 0,100" );
