...
HTML;
	...
	vass_songs.album_id = vass_albums.id LEFT JOIN vass_artists ON vass_songs.artist_id = vass_artists.id WHERE vass_songs.title LIKE '%$q%' LIMIT $start,$offset");
	}else{
	$db->query("SELECT vass_songs.recent, vass_songs.played, vass_songs.loved, vass_songs.id AS song_id, vass_songs.loved, vass_songs.title AS song_title, 
	vass_artists.name AS song_artist, vass_albums.name AS song_album, vass_albums.id AS album_id FROM vass_songs LEFT JOIN vass_albums ON 
	vass_songs.album_id = vass_albums.id LEFT JOIN vass_artists ON vass_songs.artist_id = vass_artists.id ORDER BY vass_songs.id DESC LIMIT $start,$offset");
	}
...
HTML;
	...
	if($q){
		...
		$pages = navigation("admin/index.php?do=songs&q=" . urlencode($q) . "&p={page}", $total['count'], $offset);
	}else{
		$total = $db->super_query("SELECT COUNT(*) AS count FROM vass_songs");
		$pages = navigation("admin/index.php?do=songs&p={page}", $total['count'], $offset);
	}
