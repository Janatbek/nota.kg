...
function feat_artists($feat_artist){
global $config, $logged, $db;
$feat_artist = explode(",",$feat_artist);
if($feat_artist){
$artists_query = $db->query("SELECT name, id FROM vass_artists WHERE id REGEXP '[[:<:]]" . $feat_artist['0'] . "[[:>:]]' LIMIT 0,10;");
while($artists = $db->get_row($artists_query)){
$buffer[] = $artists;
}
}else $buffer = array();
