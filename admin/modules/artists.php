...
HTML;
	...
	if($q){
		$db->query("SELECT * FROM vass_artists WHERE name LIKE '%$q%' LIMIT $start,$offset");
	}else{
		$db->query("SELECT * FROM vass_artists ORDER BY id DESC LIMIT $start,$offset ");
	}
	...
	if($q){
		...
		$pages = navigation("admin/index.php?do=artists&q=" . urlencode($q) . "&p={page}", $total['count'], $offset);
	}else{
		$total = $db->super_query("SELECT COUNT(*) AS count FROM vass_artists");
		$pages = navigation("admin/index.php?do=artists&p={page}", $total['count'], $offset);
	}
